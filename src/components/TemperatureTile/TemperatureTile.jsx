...
const TemperatureTile = ({ current, day, settings }) => {
  ...
  return (
    <Tile label="Temperature" meta={`°${unit}`} className="t-temp">
      ...
      <div className="temp-sub">
        <span>Feels <b>{fixed1(tempValue(current.feelslike_c, unit))}°</b></span>
        <span>Dew <b>{current.dewpoint_c != null ? `${fixed1(tempValue(current.dewpoint_c, unit))}°` : '--'}</b></span>
      </div>
